...
func (r *postgresInvite) Create(ctx context.Context, invite domain.SchoolInvite) error {
	_, err := r.db.Exec(ctx, `
		INSERT INTO school_invites (id, school_name, email, token, expires_at, phone_number, temp_password)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
	`, invite.ID, invite.SchoolName, invite.Email, invite.Token, invite.ExpiresAt, invite.PhoneNumber, invite.TempPassword)
	return err
}
...
func (r *postgresInvite) GetByToken(ctx context.Context, token string) (*domain.SchoolInvite, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT id, school_name, email, token, expires_at, accepted_at, created_at,
		       COALESCE(phone_number, ''), COALESCE(temp_password, '')
		FROM school_invites WHERE token = $1
	`, token).Scan(&inv.ID, &inv.SchoolName, &inv.Email, &inv.Token, &inv.ExpiresAt, &inv.AcceptedAt, &inv.CreatedAt, &inv.PhoneNumber, &inv.TempPassword)
	if err != nil {
		return nil, err
	}
	return &inv, nil
...
func (r *postgresInvite) GetAll(ctx context.Context) ([]domain.SchoolInvite, error) {
	...
	for rows.Next() {
		var inv domain.SchoolInvite
		if err := rows.Scan(&inv.ID, &inv.SchoolName, &inv.Email, &inv.Token, &inv.ExpiresAt, &inv.AcceptedAt, &inv.CreatedAt, &inv.PhoneNumber, &inv.TempPassword); err != nil {
			return nil, err
		}
