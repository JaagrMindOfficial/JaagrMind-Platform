...
func (r *postgresSchool) Create(ctx context.Context, req domain.CreateSchoolRequest) (*domain.School, error) {
	...
	err := r.db.QueryRow(ctx, `
		INSERT INTO schools (name, school_code, city, state, contact, phone_number, parent_school_id)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id, name, school_code, city, COALESCE(state, ''), contact, COALESCE(phone_number, ''), COALESCE(logo, ''), parent_school_id, is_active, is_blocked, created_at
	`, req.Name, req.SchoolCode, req.City, req.State, req.Contact, req.PhoneNumber, req.ParentSchoolID).Scan(
		&school.ID, &school.Name, &school.SchoolCode, &school.City, &school.State, &school.Contact, &school.PhoneNumber, &school.Logo, &school.ParentSchoolID, &school.IsActive, &school.IsBlocked, &school.CreatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *postgresSchool) GetByID(ctx context.Context, id string) (*domain.School, error) {
	var school domain.School
	err := r.db.QueryRow(ctx, `
		SELECT id, name, school_code, city, COALESCE(state, ''), contact, COALESCE(phone_number, ''), COALESCE(logo, ''), parent_school_id, is_active, is_blocked, created_at
...
func (r *postgresSchool) GetAll(ctx context.Context) ([]domain.School, error) {
	...
	for rows.Next() {
		var s domain.School
		if err := rows.Scan(
			&s.ID, &s.Name, &s.SchoolCode, &s.City, &s.State, &s.Contact, &s.PhoneNumber, &s.Logo, &s.ParentSchoolID, &s.IsActive, &s.IsBlocked, &s.CreatedAt,
		); err != nil {
			return nil, err
		}
...
func (r *postgresSchool) Update(ctx context.Context, id string, req domain.CreateSchoolRequest) (*domain.School, error) {
	...
	err := r.db.QueryRow(ctx, `
		UPDATE schools
		SET name = $1, school_code = $2, city = $3, state = $4, contact = $5, phone_number = $6, parent_school_id = $7
		WHERE id = $8
		RETURNING id, name, school_code, city, COALESCE(state, ''), contact, COALESCE(phone_number, ''), COALESCE(logo, ''), parent_school_id, is_active, is_blocked, created_at
	`, req.Name, req.SchoolCode, req.City, req.State, req.Contact, req.PhoneNumber, req.ParentSchoolID, id).Scan(
		&school.ID, &school.Name, &school.SchoolCode, &school.City, &school.State, &school.Contact, &school.PhoneNumber, &school.Logo, &school.ParentSchoolID, &school.IsActive, &school.IsBlocked, &school.CreatedAt,
	)
	if err != nil {
		return nil, err
